...
A Flokicoin-focused blockchain index engine and HTTP API written in Rust, forked from [Blockstream's Esplora electrs](https://github.com/Blockstream/electrs) and ultimately based on [romanz/electrs](https://github.com/romanz/electrs).
...
Install Rust, Flokicoin Core (no `txindex` needed) and the `clang` and `cmake` packages, increase maximum number open files by `ulimit -n 100000` and then:

```bash
$ git clone https://github.com/flokiorg/flokicoin-electrs.git && cd flokicoin-electrs
$ cargo run --release --bin electrs -- -vvvv --daemon-dir ~/.flokicoin
...
This fork continues to be distributed under the upstream MIT license.
