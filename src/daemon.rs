...
fn numeric_verbosity(verbose: bool) -> u32 {
    if verbose {
        2
    } else {
        0
...
impl Connection {
    ...
    fn send(&mut self, request: &str) -> Result<()> {
        // debug!("rpc request body: {}", request);
        let cookie = &self.cookie_getter.get()?;
        let encoded_cookie = BASE64_STANDARD.encode(cookie);
        ...
        //     debug!(
        //         "rpc curl equivalent: curl --verbose --request POST http://{}/ \\
        // --header 'Authorization: Basic {}' \\
        // --header 'Content-Type: application/json' \\
        // --data '{}'",
        //         self.addr,
        //         encoded_cookie,
        //         shell_escape_single_quotes(request),
        //     );
    ...
    fn recv(&mut self) -> Result<String> {
        ...
        } else if status == "HTTP/1.1 503 Service Unavailable" {
            warn!("RPC HTTP 503 error: {}", contents);
            bail!(ErrorKind::Connection(format!(
                "daemon reported 503 Service Unavailable: {}",
                contents
            )));
        } else {
            bail!(
...
impl Daemon {
    ...
    pub fn read_blk_file_xor_key(&self) -> Result<Option<[u8; 8]>> {
        // From: <https://github.com/flokicoin/flokicoin/blob/v28.0/src/node/blockstorage.cpp#L1160>
    ...
    pub fn getblock(&self, blockhash: &BlockHash) -> Result<Block> {
        let block = block_from_value(self.getblock_with_backcompat(blockhash, false)?)?;
        assert_eq!(block.block_hash(), *blockhash);
        Ok(block)
    }

    fn getblock_with_backcompat(&self, blockhash: &BlockHash, verbose: bool) -> Result<Value> {
        let params_numeric = json!([blockhash, numeric_verbosity(verbose)]);
        match self.request("getblock", params_numeric) {
            Err(Error(ErrorKind::RpcError(code, msg, method), _))
                if method == "getblock" && code == -32602 && msg.contains("verbosity") =>
            {
                debug!(
                    "retrying getblock with boolean verbosity after RPC error: {}",
                    msg
                );
    ...
    pub fn getblocks(&self, blockhashes: &[BlockHash]) -> Result<Vec<Block>> {
        let verbose = false;
        let params_numeric: Vec<Value> = blockhashes
            .iter()
            .map(|hash| json!([hash, numeric_verbosity(verbose)]))
            .collect();
        let params_bool: Vec<Value> = blockhashes
            .iter()
            .map(|hash| json!([hash, verbose]))
            .collect();
        let mut params_list = params_numeric.clone();
        let mut using_bool_verbosity = false;
            ...
            match self.requests("getblock", params_list.clone()) {
                ...
                Err(Error(ErrorKind::RpcError(code, msg, method), _))
                    if method == "getblock"
                        ...
                        && !using_bool_verbosity =>
                {
                    debug!(
                        "reissuing getblock batch with boolean verbosity after RPC error: {}",
                        msg
                    );
                    params_list = params_bool.clone();
                    using_bool_verbosity = true;
                    attempts += 1; // do not count compatibility retry against attempts
                    continue;
                }
                Err(e) => {
